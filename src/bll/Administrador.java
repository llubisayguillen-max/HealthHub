...
public class Administrador extends Usuario implements Menu {
	...
	public void MostrarMenu() {
		String[] opciones = { "Alta paciente", "Alta médico", "Baja paciente", "Baja médico", "Crear historia médica",
				"Reset contraseña", "Salir" };

		int elegido = JOptionPane.showOptionDialog(null, "Elija opción", "Administrador", JOptionPane.DEFAULT_OPTION,
		...
		case 4:
			...
			Paciente pacHist = sistema.getPacientes().stream().filter(pa -> pa.getUsuario().equals(usuarioHist))
					.findFirst().orElse(null);
			if (pacHist != null) {
				String diag = JOptionPane.showInputDialog("Diagnóstico:");
				HistorialMedico hist = new HistorialMedico(0, new java.util.Date(), diag, new String[] {});
		...
		case 5:
			...
			Usuario userR = sistema.getUsuarios().stream().filter(u -> u.getUsuario().equals(usuarioR)).findFirst()
					.orElse(null);
			if (userR != null) {
